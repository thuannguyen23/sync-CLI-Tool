...
- Use `ctx_execute(language, code)` or `ctx_execute_file(path, language, code)` to process data inside the sandbox and log only the final summary/answer.
- Use `ctx_batch_execute(commands, queries)` to batch-run multiple commands and auto-index results.
- Use `ctx_fetch_and_index(url, source)` instead of raw `curl` or full HTML dumps.
